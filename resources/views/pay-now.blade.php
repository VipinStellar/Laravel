...
<body>
    ...
    <section class="pt-1 pb-4 job-form loading" id="pay-form"> 
     @if($result['status'] !='error' && ($result['sez']=='no') && (isset($result['customer_details']) && $result['customer_details']['Status']=="Waiting For Payment" && $result['customer_details']['Tax_Applicable']=='No'))
        <div class="container">
          <div class="row justify-content-center">
            <div class="col-md-9 col-sm-12 col-xs-12 my-4">
              <div class="alert alert-danger py-2 fs16 text-center">
                <strong> Invoice without tax is only applicable for SEZ customers. Provided GSTIN does not belongs to SEZ : {{ $result['customer_details']['GST_IN'] }}</strong>
              </div>
            </div>
          </div>
        </div>
     @elseif((isset($result['id']) && !empty($result['id'])) && (isset($result['status']) && $result['status'] !='error') && isset($result['customer_details']) && (isset($result['customer_details']['Status']) && $result['customer_details']['Status']=="Waiting For Payment")) 
        ...
        <form action="{{ $result['action'] }}" method="post" name="paymentForm" id="paymentForm">
          @csrf  
        ...
        <div class="container">
          <div class="row mb-3 pt-2">
            <div class="col-md-12 text-center">
              ...
              <h3 class="section-title mb-0" id="step-title">Provide Your Billing Details</h3>
            </div>
          </div>
          <div class="row justify-content-center">
            <div class="col-sm-12 col-md-12 col-lg-11 col-xl-10 pb-4">
              <div class="row">
              <!-- For Step 1 Start-->
                <div class="col-sm-7 col-md-8 pr-xl-5 pl-md-5 pl-lg-3 order-2 order-sm-1" id="step-info" data-title="Provide Your Billing Details">
                    ...
                    <div class="form-row">
                        ...
                        <div class="col-md-6 mb-3 pr-xl-3">
                            <input type="text" class="form-control" name="landmark" id="landmark" placeholder="Landmark" value="{{ $result['customer_details']['Address2'] }}" maxlength="100">
                        </div>
                        <div class="col-md-6 mb-3 pr-xl-3 for-validation">
                            <select class="form-control" name="state" id="state" onchange="getstateId($(this))" autocomplete="off">
                                <option value="" selected data-state="0">State/ Union Territory *</option>
                                @foreach($result['states'] as $state)
                                    <option value="{{ $state['state_name'] }}" data-state="{{$state['state_code']}}">{{ $state['state_name'] }}</option>
                                @endforeach
                            </select>
                            <input type="hidden" value="" id="state_code" name="state_code" autocomplete="off" />
                        </div>
                        <div class="col-md-6 mb-3 pr-xl-3 for-validation">
                            <select name="city" id="city" class="form-control" onchange="otherCity();">
                                <option value="">Select City *</option>
                                @foreach($result['cities'] as $city)
                                    <option value="{{$city}}" {{ ($city == ucwords($result['customer_details']['City']))?'selected':'' }}>{{ $city }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6 mb-3 pr-xl-3 for-validation" id="cityname" style="display:none;">
                            <input type="text" name="other_city" id="other_city" placeholder="Enter City Name *" value="" maxlength="50" class="form-control" autocomplete="off" />
                        </div>
                        <div class="col-md-6 mb-3 pr-xl-3 for-validation">
                        ...
                        <div class="col-md-6 mb-3 pr-xl-3 company-detail-input for-validation">
                            ...
                            <input type="hidden" name="tax_amount" value="{{ $result['gst_amount'] }}">
                            <input type="hidden" name="final_amount" value="{{ $result['total_amount'] }}">
                            ...
                            <input type="hidden" name="payment_type" value="{{ $result['customer_details']['Charge_Code'] }}">
                            <input type="hidden" name="payment_category" value="service">
                            <input type="hidden" name="payment_mode" value="Payu">
                            <input type="hidden" name="payment_channel" value="Online">
                    </div>
                    ...
                    @if($errors->any())
                        <div class="alert alert-danger py-2 fs14 alert-dismissible" style="max-width: 500px;">
                    ...
                    @endif
                    <div class="alert alert-danger details-alert py-2 mt-2 fs14" style="display:none; max-width:500px;"></div>
                    <button type="button" id="GoToStepDetails" class="btn btn-primary pl-md-5 pr-md-5 pl-4 pr-4 fs16 mb-3 mt-3">Verify Details <i class="bi bi-arrow-right"></i></button>
                    <p class="mandatory pt-1">( * ) Required Fields </p>
                  </div>
                </div>
            </div>
              <!-- For Step 1 End-->
              <!-- For Step 2 Start -->
                <div class="col-sm-7 col-md-8 mb-3 order-2 order-sm-1" id="step-details" data-title="Verify Your Details" style="display:none">
                    <div class="card bill-info">
                    <div class="card-header text-left">
                    ...
                    <div class="card-body pr-xl-2">
                        ...
                        <button type="submit" class="btn btn-primary pl-md-5 pr-md-5 pl-4 pr-4 mt-1 mb-2 fs16"  name="pay_now" value="proceed">Proceed To Pay <i class="bi bi-arrow-right"></i></button>
                        <p><a href="javascript:void(0);" id="BackToStepInfo" class="text-muted"><strong><i class="bi bi-arrow-left"></i> Go Back</strong></a></p>
                     </div>
                    </div>
                </div>
                <!-- For Step 2 End-->
                <div class="col-sm-5 col-md-4 pl-sm-0 pl-md-3 pl-xl-4 mb-5 order-1 order-sm-2" id="preview-box">
                  <div class="sticky">
                    <div class="border dark-bg px-3 pt-4 pb-3 preview_fee">
                      <p class="h6"><span class="text_fee_type">Data Recovery Fee</span>: <span class="float-right"><em class="rupee">`</em> <strong class="subtotal_amount">{{ $result['customer_details']['Base_Amount'] }}</strong></span></p>
                      <p class="h6">GST @<span class="tax_rate">18</span>%: <span class="float-right"><em class="rupee">`</em> <strong class="tax_amount">{{ $result['gst_amount'] }}</strong></span></p>
                      <hr>
                      <p class="h6"><strong>Total Amount:</strong> <strong class="float-right h4 fw600"><em class="rupee">`</em> <span class="final_amount">{{ $result['total_amount'] }}</span></strong></p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      @else
        <div class="container">
          <div class="row justify-content-center">
            <div class="col-md-9 col-sm-12 col-xs-12 my-4">
              <div class="alert alert-danger py-2 fs16 text-center">
                <strong>{{ ((isset($result['status']) && $result['status']=='error') && isset($result['customer_details']['meassage'])) ? $result['customer_details']:'Your Payment Details was Not Found!' }}</strong>
              </div>
            </div>
          </div>
        </div>    
...
@include('footer')
<script defer src="{{ url('public/js/pay-now.js?v=3') }}"></script>
<script>
  $(function(){
